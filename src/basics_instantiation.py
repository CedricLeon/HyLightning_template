...
class MyModel:
    def __init__(self, name: str, nb_channels: int, loss: str, lr: float = 0.001):
        ...
        self.lr = lr
        print(
            f"Initialized MyModel with name={name}, nb_channels={nb_channels}, loss={loss}, lr={lr}"
        )
...
class MyBIGModel:
    def __init__(self, name: str, nb_channels: int, loss: str, optimizer: torch.optim.Optimizer):
        ...
        print(
            f"Initialized MyBIGModel with name={name}, nb_channels={nb_channels}, loss={loss}, optimizer={optimizer.__class__.__name__}"
        )
...
def my_app(cfg: DictConfig) -> None:
    ...
    if cfg.get("model") is not None:
        ...
        if type(model).__name__ in ("MyModel", "MyBIGModel"):
            print(model)
        else:
